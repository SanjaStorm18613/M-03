...
public class CalibrationAutonomousBase {
    ...
    public CalibrationAutonomousBase(LinearOpMode opMode, ContantsAuto cAuto) {
    ...
    public void execution() {
        ...
        for (int action = 0; action <= idxActionSteps; action++) {
            ...
            if (steps.get(0).get(action)[1].equals(cAuto.DR)) {
                drive.setMove(false, steps.get(0).get(action)[2].equals(cAuto.DR_SIDE), 0.5, 800, 0.0005, steps.get(0).get(action)[0], 0);
                drive.setValCalibration(drAdjust);
                opMode.telemetry.addData("drAdjust", drAdjust);

            } else if (steps.get(0).get(action)[1].equals(cAuto.DR_TURN)) {
                drive.setMove(true, false, 0.5, 800, 0.0005, 0, steps.get(0).get(action)[0] + drTurnAdjust);
                opMode.telemetry.addData("drTurnAdjust", drTurnAdjust);
                ...
                opMode.telemetry.addData("elAdjust", elAdjust);
                ...
                opMode.telemetry.addData("ywAdjust", ywAdjust);
